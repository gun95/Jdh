...
namespace Jdh
{
    class Program
    {
       ...
       static Corale.Colore.Razer.Keyboard.Key[] KeyD = { Key.T, Key.Y, Key.U, Key.G, Key.J, Key.V, Key.B, Key.N};
        ...
        static void Wave(Key key)
        {
            Chroma.Instance.Keyboard.SetKey(key, ColoreColor.Red);
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                Chroma.Instance.Keyboard.SetKey(Key.Z, ColoreColor.Red);
                System.Threading.Thread.Sleep(1000);

                Chroma.Instance.Keyboard.SetKey(Key.E, ColoreColor.Red);
                System.Threading.Thread.Sleep(1000);

                Chroma.Instance.Keyboard.SetKey(Key.R, ColoreColor.Red);
                //On attends 500ms
